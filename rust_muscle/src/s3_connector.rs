...
pub fn upload_to_s3(
    ...
    let region = if let Some(ep) = endpoint {
        ...
        region_str.parse::<Region>()
            .map_err(|_| MuscleError::Generic(format!("Failed to parse S3 region: {}", region_str)))?
    };
    ...
    println!("[RUST S3] Uploading '{}' to bucket '{}' as '{}'...", file_path, bucket_name, object_key);
    ...
    rt.block_on(async {
        ...
        bucket.put_object(object_key, &contents)
            .await
...
pub fn download_from_s3(
    ...
    println!("[RUST S3] Downloading '{}' from bucket '{}' to '{}'...", object_key, bucket_name, destination);

    let rt = tokio::runtime::Runtime::new()
        .map_err(|e| MuscleError::Generic(format!("Failed to start tokio runtime: {}", e)))?;

    let response_data = rt.block_on(async {
        let res = bucket.get_object(object_key)
            .await
            .map_err(|e| MuscleError::NetworkError(format!("Failed to get object from S3: {}", e)))?;
    ...
    if let Some(parent) = dest_path.parent() {
        if !parent.exists() {
            std::fs::create_dir_all(parent)
                .map_err(|e| MuscleError::DestDirCreation(format!("Failed to create destination directory structure: {}", e)))?;
        }
    }

    std::fs::write(dest_path, response_data)
        .map_err(|e| MuscleError::PermissionDenied(format!("Failed to write downloaded data to destination file: {}", e)))?;
