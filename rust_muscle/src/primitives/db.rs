...
pub fn handle_db_query(args: &[String]) -> Result<(), MuscleError> {
        ...
        match args[i].as_str() {
            ...
            "--query" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --query".to_string(),
                    ));
                }
            }
            "--destination" => {
    ...
    let connection_string = connection_string.ok_or_else(|| {
        MuscleError::MissingArg("Missing argument --connection-string".to_string())
    })?;
    let query =
        query.ok_or_else(|| MuscleError::MissingArg("Missing argument --query".to_string()))?;
    ...
    rt.block_on(async { db_connector::query_to_file(connection_string, query, destination).await })
...
pub fn handle_db_insert(args: &[String]) -> Result<(), MuscleError> {
        ...
        match args[i].as_str() {
            ...
            "--table-name" | "--table_name" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --table-name".to_string(),
                    ));
                }
            }
            "--source" => {
            ...
            "--schema-drift" | "--schema_drift" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --schema-drift".to_string(),
                    ));
                }
            }
            other => {
    ...
    let table_name = table_name
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --table-name".to_string()))?;
    let source =
    ...
    rt.block_on(async {
        db_connector::insert_from_file(connection_string, table_name, source, &mode, schema_drift)
            .await
    })
    .map_err(|e| MuscleError::IoError(format!("DB error: {}", e)))
}
...
pub fn handle_db_upsert(args: &[String]) -> Result<(), MuscleError> {
    ...
    rt.block_on(async {
        db_connector::upsert_from_file(connection_string, table_name, source, keys, schema_drift)
            .await
    })
    .map_err(|e| MuscleError::IoError(format!("DB error: {}", e)))
}
