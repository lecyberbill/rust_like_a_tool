...
pub fn handle_data_groupby(args: &[String]) -> Result<(), MuscleError> {
        ...
        match args[i].as_str() {
            ...
            "--groupby-columns" | "--groupby_columns" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --groupby-columns".to_string(),
                    ));
                }
            }
            "--aggregate-column" | "--aggregate_column" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --aggregate-column".to_string(),
                    ));
                }
            }
            "--operation" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --operation".to_string(),
                    ));
                }
            }
            other => {
    ...
    let groupby_str = groupby_columns
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --groupby-columns".to_string()))?;
    let agg_col = aggregate_column.ok_or_else(|| {
        MuscleError::MissingArg("Missing argument --aggregate-column".to_string())
    })?;
    let op = operation
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --operation".to_string()))?;

    let groupby_cols: Vec<String> = groupby_str
        .split(',')
...
pub fn handle_data_join(args: &[String]) -> Result<(), MuscleError> {
        ...
        match args[i].as_str() {
            "--left-source" | "--left_source" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --left-source".to_string(),
                    ));
                }
            }
            ...
            "--how" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --how".to_string(),
                    ));
                }
            }
            other => {
    ...
    let left_source = left_source
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --left-source".to_string()))?;
    let right_source = right_source
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --right-source".to_string()))?;
    let destination = destination
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --destination".to_string()))?;
    let left_on =
        left_on.ok_or_else(|| MuscleError::MissingArg("Missing argument --left-on".to_string()))?;
    let right_on = right_on
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --right-on".to_string()))?;

    analytical_engine::join(
        left_source,
        right_source,
        destination,
        left_on,
...
pub fn handle_data_metrics(args: &[String]) -> Result<(), MuscleError> {
        ...
        match args[i].as_str() {
            ...
            "--regex-pattern" | "--regex_pattern" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --regex-pattern".to_string(),
                    ));
                }
            }
            "--limit-rows" | "--limit_rows" => {
                if i + 1 < args.len() {
                    let parsed = args[i + 1].parse::<usize>().map_err(|_| {
                        MuscleError::InvalidArg("Invalid integer for --limit-rows".to_string())
                    })?;
                    limit_rows = Some(parsed);
                    i += 2;
                } else {
                    return Err(MuscleError::MissingArg(
                        "Missing value for --limit-rows".to_string(),
                    ));
                }
            }
    ...
    let column_name = column_name
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --column-name".to_string()))?;
...
pub fn handle_data_lookup(args: &[String]) -> Result<(), MuscleError> {
        ...
        match args[i].as_str() {
            ...
            "--lookup-file" | "--lookup_file" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --lookup-file".to_string(),
                    ));
                }
            }
            "--source-key" | "--source_key" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --source-key".to_string(),
                    ));
                }
            }
            "--lookup-key" | "--lookup_key" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --lookup-key".to_string(),
                    ));
                }
            }
            "--lookup-value" | "--lookup_value" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --lookup-value".to_string(),
                    ));
                }
            }
            "--destination" => {
    ...
    let source =
        source.ok_or_else(|| MuscleError::MissingArg("Missing argument --source".to_string()))?;
    let lookup_file = lookup_file
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --lookup-file".to_string()))?;
    let source_key = source_key
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --source-key".to_string()))?;
    let lookup_key = lookup_key
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --lookup-key".to_string()))?;
    let lookup_value = lookup_value
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --lookup-value".to_string()))?;
    let destination = destination
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --destination".to_string()))?;

    analytical_engine::lookup(
        source,
        lookup_file,
        source_key,
        lookup_key,
...
pub fn handle_data_deduplicate(args: &[String]) -> Result<(), MuscleError> {
        ...
        match args[i].as_str() {
            ...
            "--subset" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --subset".to_string(),
                    ));
                }
            }
            "--keep" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --keep".to_string(),
                    ));
                }
            }
            other => {
    ...
    let destination = destination
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --destination".to_string()))?;
    let subset_str =
        subset.ok_or_else(|| MuscleError::MissingArg("Missing argument --subset".to_string()))?;

    let subset_cols: Vec<String> = subset_str
        .split(',')
        .map(|s| s.trim().to_string())
        .collect();
...
pub fn handle_data_anonymize(args: &[String]) -> Result<(), MuscleError> {
    ...
    let destination = destination
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --destination".to_string()))?;
    let rules_str =
        rules.ok_or_else(|| MuscleError::MissingArg("Missing argument --rules".to_string()))?;
    ...
    for rule in rules_str.split(',') {
        ...
        if parts.len() != 2 {
            return Err(MuscleError::InvalidArg(format!(
                "Invalid rule format: '{}'. Expected col:strategy",
                rule
            )));
        }
...
pub fn handle_data_pivot(args: &[String]) -> Result<(), MuscleError> {
        ...
        match args[i].as_str() {
            ...
            "--index" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --index".to_string(),
                    ));
                }
            }
            "--on" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --on".to_string(),
                    ));
                }
            }
            "--values" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --values".to_string(),
                    ));
                }
            }
            "--aggregate" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --aggregate".to_string(),
                    ));
                }
            }
            other => {
    ...
    let destination = destination
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --destination".to_string()))?;
    let index_str =
        index.ok_or_else(|| MuscleError::MissingArg("Missing argument --index".to_string()))?;
    let on = on.ok_or_else(|| MuscleError::MissingArg("Missing argument --on".to_string()))?;
    let values =
        values.ok_or_else(|| MuscleError::MissingArg("Missing argument --values".to_string()))?;
...
pub fn handle_data_unpivot(args: &[String]) -> Result<(), MuscleError> {
        ...
        match args[i].as_str() {
            ...
            "--variable-name" | "--variable_name" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --variable-name".to_string(),
                    ));
                }
            }
            "--value-name" | "--value_name" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --value-name".to_string(),
                    ));
                }
            }
            other => {
    ...
    let index_cols: Vec<String> = index_str.split(',').map(|s| s.trim().to_string()).collect();
    let on_cols: Option<Vec<String>> =
        on.map(|s| s.split(',').map(|c| c.trim().to_string()).collect());

    analytical_engine::unpivot(
        source,
        destination,
        index_cols,
        on_cols,
...
pub fn handle_data_delta(args: &[String]) -> Result<(), MuscleError> {
        ...
        match args[i].as_str() {
            ...
            "--target" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --target".to_string(),
                    ));
                }
            }
            "--keys" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --keys".to_string(),
                    ));
                }
            }
            "--destination-upsert" | "--destination_upsert" => {
                if i + 1 < args.len() {
                    destination_upsert = Some(&args[i + 1]);
                    i += 2;
                } else {
                    return Err(MuscleError::MissingArg(
                        "Missing value for --destination-upsert".to_string(),
                    ));
                }
            }
            "--destination-delete" | "--destination_delete" => {
                if i + 1 < args.len() {
                    destination_delete = Some(&args[i + 1]);
                    i += 2;
                } else {
                    return Err(MuscleError::MissingArg(
                        "Missing value for --destination-delete".to_string(),
                    ));
                }
            }
            "--destination-sync" | "--destination_sync" => {
                if i + 1 < args.len() {
                    destination_sync = Some(&args[i + 1]);
                    i += 2;
                } else {
                    return Err(MuscleError::MissingArg(
                        "Missing value for --destination-sync".to_string(),
                    ));
                }
            }
            other => {
    ...
    let source =
        source.ok_or_else(|| MuscleError::MissingArg("Missing argument --source".to_string()))?;
    let target =
        target.ok_or_else(|| MuscleError::MissingArg("Missing argument --target".to_string()))?;
    let keys_str =
        keys.ok_or_else(|| MuscleError::MissingArg("Missing argument --keys".to_string()))?;
    let dest_upsert = destination_upsert.ok_or_else(|| {
        MuscleError::MissingArg("Missing argument --destination-upsert".to_string())
    })?;
    let dest_delete = destination_delete.ok_or_else(|| {
        MuscleError::MissingArg("Missing argument --destination-delete".to_string())
    })?;

    let keys_vec: Vec<String> = keys_str.split(',').map(|s| s.trim().to_string()).collect();

    analytical_engine::delta(
        source,
        target,
        keys_vec,
        dest_upsert,
        dest_delete,
        destination_sync.map(|s| s.as_str()),
    )
    .map_err(|e| MuscleError::IoError(e))
}
...
pub fn handle_data_type_cast(args: &[String]) -> Result<(), MuscleError> {
        ...
        match args[i].as_str() {
            ...
            "--casts" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg(
                        "Missing value for --casts".to_string(),
                    ));
                }
            }
            other => {
    ...
    let destination = destination
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --destination".to_string()))?;
    let casts_str =
        casts.ok_or_else(|| MuscleError::MissingArg("Missing argument --casts".to_string()))?;
...
pub fn handle_data_partition(args: &[String]) -> Result<(), MuscleError> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--destination-dir" | "--destination_dir" => {
                if i + 1 < args.len() {
                    destination_dir = Some(&args[i + 1]);
                    i += 2;
                } else {
                    return Err(MuscleError::MissingArg(
                        "Missing value for --destination-dir".to_string(),
                    ));
                }
            }
            "--by-columns" | "--by_columns" => {
                if i + 1 < args.len() {
                    by_columns = Some(&args[i + 1]);
                    i += 2;
                } else {
                    return Err(MuscleError::MissingArg(
                        "Missing value for --by-columns".to_string(),
                    ));
                }
            }
            other => {
    ...
    let dest_dir = destination_dir
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --destination-dir".to_string()))?;
    let by_cols_str = by_columns
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --by-columns".to_string()))?;

    let by_cols_vec: Vec<String> = by_cols_str
        .split(',')
        .map(|s| s.trim().to_string())
        .collect();

    analytical_engine::partition(source, dest_dir, by_cols_vec).map_err(|e| MuscleError::IoError(e))
}
...
pub fn handle_data_scd(args: &[String]) -> Result<(), MuscleError> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--compare-columns" | "--compare_columns" => {
                if i + 1 < args.len() {
                    compare_columns = Some(&args[i + 1]);
                    i += 2;
                } else {
                    return Err(MuscleError::MissingArg(
                        "Missing value for --compare-columns".to_string(),
                    ));
                }
            }
            "--destination" => {
            ...
            "--valid-from-col" | "--valid_from_col" => {
                if i + 1 < args.len() {
                    valid_from_col = args[i + 1].clone();
                    i += 2;
                } else {
                    return Err(MuscleError::MissingArg(
                        "Missing value for --valid-from-col".to_string(),
                    ));
                }
            }
            "--valid-to-col" | "--valid_to_col" => {
                if i + 1 < args.len() {
                    valid_to_col = args[i + 1].clone();
                    i += 2;
                } else {
                    return Err(MuscleError::MissingArg(
                        "Missing value for --valid-to-col".to_string(),
                    ));
                }
            }
            "--is-current-col" | "--is_current_col" => {
                if i + 1 < args.len() {
                    is_current_col = args[i + 1].clone();
                    i += 2;
                } else {
                    return Err(MuscleError::MissingArg(
                        "Missing value for --is-current-col".to_string(),
                    ));
                }
            }
            "--valid-from-value" | "--valid_from_value" => {
                if i + 1 < args.len() {
                    valid_from_value = Some(&args[i + 1]);
                    i += 2;
                } else {
                    return Err(MuscleError::MissingArg(
                        "Missing value for --valid-from-value".to_string(),
                    ));
                }
            }
            other => {
    ...
    let destination = destination
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --destination".to_string()))?;
    let valid_from_val = valid_from_value.ok_or_else(|| {
        MuscleError::MissingArg("Missing argument --valid-from-value".to_string())
    })?;
...
pub fn handle_data_split_out(args: &[String]) -> Result<(), MuscleError> {
    ...
    let destination = destination
        .ok_or_else(|| MuscleError::MissingArg("Missing argument --destination".to_string()))?;
    let column =
        column.ok_or_else(|| MuscleError::MissingArg("Missing argument --column".to_string()))?;
