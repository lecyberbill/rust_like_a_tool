...
pub fn handle_net_download(args: &[String]) -> Result<(), MuscleError> {
        ...
        match args[i].as_str() {
            "--url" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg("Missing value for --url".to_string()));
                }
            }
            "--destination" => {
                if i + 1 < args.len() {
    ...
    let url = url.ok_or_else(|| MuscleError::MissingArg("Missing required argument --url".to_string()))?;
...
fn download_file(url: &str, destination: &str) -> Result<(), MuscleError> {
    ...
    response.copy_to(&mut dest_file)
        .map_err(|e| MuscleError::IoError(format!("Failed to write downloaded data to file: {}", e)))?;
...
pub fn handle_net_upload(args: &[String]) -> Result<(), MuscleError> {
        ...
        match args[i].as_str() {
            "--file_path" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg("Missing value for --file_path".to_string()));
                }
            }
            "--url" => {
                if i + 1 < args.len() {
            ...
            "--method" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg("Missing value for --method".to_string()));
                }
            }
            "--headers" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg("Missing value for --headers".to_string()));
                }
            }
            other => {
                return Err(MuscleError::InvalidArg(format!("Unknown argument '{}'", other)));
    ...
    let file_path = file_path.ok_or_else(|| MuscleError::MissingArg("Missing required argument --file_path".to_string()))?;
    let url = url.ok_or_else(|| MuscleError::MissingArg("Missing required argument --url".to_string()))?;
...
pub fn handle_net_http_request(args: &[String]) -> Result<(), MuscleError> {
        ...
        match args[i].as_str() {
            ...
            "--body" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg("Missing value for --body".to_string()));
                }
            }
            "--extract-regex" | "--extract_regex" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg("Missing value for --extract-regex".to_string()));
                }
            }
            "--extract-destination" | "--extract_destination" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg("Missing value for --extract-destination".to_string()));
                }
            }
            other => {
                return Err(MuscleError::InvalidArg(format!("Unknown argument '{}'", other)));
    ...
    if let Some(regex_str) = extract_regex {
        let ext_dest = extract_destination.ok_or_else(|| {
            MuscleError::MissingArg("Missing argument --extract-destination required by --extract-regex".to_string())
        })?;
        ...
        let base_url = reqwest::Url::parse(url)
            .map_err(|e| MuscleError::IoError(format!("Failed to parse base URL for relative resolution: {}", e)))?;
        ...
        let json_bytes = serde_json::to_vec_pretty(&matches)
            .map_err(|e| MuscleError::IoError(format!("Failed to serialize matches list: {}", e)))?;
...
fn run_ftp_helper(
    ...
    if output.status.success() {
        ...
        Err(MuscleError::IoError(format!("FTP helper execution failed: {}", stderr.trim())))
    }
}
...
pub fn handle_net_ftp_download(args: &[String]) -> Result<(), MuscleError> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--host" => {
                ...
                else { return Err(MuscleError::MissingArg("Missing value for --host".to_string())); }
            }
            "--port" => {
                if i + 1 < args.len() { port = args[i+1].clone(); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --port".to_string())); }
            }
            "--user" => {
                if i + 1 < args.len() { user = Some(&args[i+1]); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --user".to_string())); }
            }
            "--password" => {
                if i + 1 < args.len() { password = Some(&args[i+1]); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --password".to_string())); }
            }
            "--remote-path" | "--remote_path" => {
                if i + 1 < args.len() { remote_path = Some(&args[i+1]); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --remote-path".to_string())); }
            }
            "--local-path" | "--local_path" => {
                if i + 1 < args.len() { local_path = Some(&args[i+1]); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --local-path".to_string())); }
            }
            other => {
                return Err(MuscleError::InvalidArg(format!("Unknown argument '{}'", other)));
            }
    ...
    let host = host.ok_or_else(|| MuscleError::MissingArg("Missing required argument --host".to_string()))?;
    let user = user.ok_or_else(|| MuscleError::MissingArg("Missing required argument --user".to_string()))?;
    let password = password.ok_or_else(|| MuscleError::MissingArg("Missing required argument --password".to_string()))?;
    let remote_path = remote_path.ok_or_else(|| MuscleError::MissingArg("Missing required argument --remote-path".to_string()))?;
    let local_path = local_path.ok_or_else(|| MuscleError::MissingArg("Missing required argument --local-path".to_string()))?;
...
fn run_sftp_helper(
    ...
    if output.status.success() {
        ...
        let stderr = String::from_utf8_lossy(&output.stderr);
        Err(MuscleError::IoError(format!("SFTP helper execution failed: {}", stderr.trim())))
    }
}
...
pub fn handle_net_sftp_download(args: &[String]) -> Result<(), MuscleError> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--key-path" | "--key_path" => {
                ...
                else { return Err(MuscleError::MissingArg("Missing value for --key-path".to_string())); }
            }
            "--key-passphrase" | "--key_passphrase" => {
                if i + 1 < args.len() { key_passphrase = args[i+1].clone(); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --key-passphrase".to_string())); }
            }
            other => {
                return Err(MuscleError::InvalidArg(format!("Unknown argument '{}'", other)));
            }
    ...
    if password.is_empty() && key_path.is_empty() {
        return Err(MuscleError::MissingArg("Either --password or --key-path must be provided for SFTP authentication".to_string()));
    }
...
fn run_google_sheets_helper(
    ...
    let output = cmd
        ...
        .output()
        .map_err(|e| MuscleError::IoError(format!("Failed to start Python google_sheets_helper: {}", e)))?;
    ...
    if output.status.success() {
        ...
        let stderr = String::from_utf8_lossy(&output.stderr);
        Err(MuscleError::IoError(format!("Google Sheets helper execution failed: {}", stderr.trim())))
    }
}
...
pub fn handle_google_sheets_read(args: &[String]) -> Result<(), MuscleError> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--credentials" => {
                ...
                else { return Err(MuscleError::MissingArg("Missing value for --credentials".to_string())); }
            }
            "--spreadsheet-id" | "--spreadsheet_id" => {
                if i + 1 < args.len() { spreadsheet_id = Some(&args[i+1]); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --spreadsheet-id".to_string())); }
            }
            "--worksheet-title" | "--worksheet_title" => {
                if i + 1 < args.len() { worksheet_title = args[i+1].clone(); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --worksheet-title".to_string())); }
            }
            "--local-path" | "--local_path" => {
                if i + 1 < args.len() { local_path = Some(&args[i+1]); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --local-path".to_string())); }
    ...
    let credentials = credentials.ok_or_else(|| MuscleError::MissingArg("Missing required argument --credentials".to_string()))?;
    let spreadsheet_id = spreadsheet_id.ok_or_else(|| MuscleError::MissingArg("Missing required argument --spreadsheet-id".to_string()))?;
    let local_path = local_path.ok_or_else(|| MuscleError::MissingArg("Missing required argument --local-path".to_string()))?;
        ...
        match args[i].as_str() {
            ...
            "--clear-sheet" | "--clear_sheet" => {
                if i + 1 < args.len() {
                    ...
                    return Err(MuscleError::MissingArg("Missing value for --clear-sheet".to_string()));
                }
            }
            other => {
                return Err(MuscleError::InvalidArg(format!("Unknown argument '{}'", other)));
...
fn run_notify_helper(
    ...
    let output = cmd.output()
        .map_err(|e| MuscleError::IoError(format!("Failed to start Python notify helper: {}", e)))?;
    ...
    if output.status.success() {
        ...
        let stderr = String::from_utf8_lossy(&output.stderr);
        Err(MuscleError::IoError(format!("Notify helper execution failed: {}", stderr.trim())))
    }
}
...
pub fn handle_net_notify(args: &[String]) -> Result<(), MuscleError> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--type" => {
                ...
                else { return Err(MuscleError::MissingArg("Missing value for --type".to_string())); }
            }
            "--smtp-host" | "--smtp_host" => {
                if i + 1 < args.len() { smtp_host = args[i+1].clone(); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --smtp-host".to_string())); }
            }
            "--smtp-port" | "--smtp_port" => {
                if i + 1 < args.len() { smtp_port = args[i+1].clone(); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --smtp-port".to_string())); }
            }
            "--smtp-user" | "--smtp_user" => {
                if i + 1 < args.len() { smtp_user = args[i+1].clone(); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --smtp-user".to_string())); }
            }
            "--smtp-pass" | "--smtp_pass" => {
                if i + 1 < args.len() { smtp_pass = args[i+1].clone(); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --smtp-pass".to_string())); }
            }
            "--to" => {
                if i + 1 < args.len() { to = Some(&args[i+1]); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --to".to_string())); }
            }
            "--subject" => {
                if i + 1 < args.len() { subject = args[i+1].clone(); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --subject".to_string())); }
            }
            "--url" => {
                if i + 1 < args.len() { url = Some(&args[i+1]); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --url".to_string())); }
            }
            "--message" => {
                if i + 1 < args.len() { message = Some(&args[i+1]); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --message".to_string())); }
            }
            other => {
                return Err(MuscleError::InvalidArg(format!("Unknown argument '{}'", other)));
            }
    ...
    let notify_type = notify_type.ok_or_else(|| MuscleError::MissingArg("Missing required argument --type".to_string()))?;
    let message = message.ok_or_else(|| MuscleError::MissingArg("Missing required argument --message".to_string()))?;

    match notify_type.to_lowercase().as_str() {
        "email" => {
            let to = to.ok_or_else(|| MuscleError::MissingArg("Missing required argument --to for email notification".to_string()))?;
            let run_args = [
                smtp_host.as_str(),
                smtp_port.as_str(),
                smtp_user.as_str(),
        ...
        "webhook" => {
            let url = url.ok_or_else(|| MuscleError::MissingArg("Missing required argument --url for webhook notification".to_string()))?;
            let run_args = [
                url.as_str(),
                message.as_str()
            ];
        ...
        other => Err(MuscleError::InvalidArg(format!("Unsupported notification type '{}'", other)))
    }
}
...
fn run_mongodb_helper(
    ...
    let output = command_builder
        .output()
        .map_err(|e| MuscleError::IoError(format!("Failed to start Python mongodb_helper: {}", e)))?;
    ...
    if output.status.success() {
        ...
        let stderr = String::from_utf8_lossy(&output.stderr);
        Err(MuscleError::IoError(format!("MongoDB helper execution failed: {}", stderr.trim())))
    }
}
...
pub fn handle_mongodb_find(args: &[String]) -> Result<(), MuscleError> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--connection-string" | "--connection_string" => {
                ...
                else { return Err(MuscleError::MissingArg("Missing value for --connection-string".to_string())); }
            }
            "--database" => {
                if i + 1 < args.len() { database = Some(&args[i+1]); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --database".to_string())); }
            }
            "--collection" => {
                if i + 1 < args.len() { collection = Some(&args[i+1]); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --collection".to_string())); }
            }
            "--filter" => {
                if i + 1 < args.len() { filter = args[i+1].clone(); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --filter".to_string())); }
            }
            "--projection" => {
                if i + 1 < args.len() { projection = args[i+1].clone(); i += 2; }
                else { return Err(MuscleError::MissingArg("Missing value for --projection".to_string())); }
            }
            "--destination" => {
    ...
    let connection_string = connection_string.ok_or_else(|| MuscleError::MissingArg("Missing required argument --connection-string".to_string()))?;
    let database = database.ok_or_else(|| MuscleError::MissingArg("Missing required argument --database".to_string()))?;
    let collection = collection.ok_or_else(|| MuscleError::MissingArg("Missing required argument --collection".to_string()))?;
    let destination = destination.ok_or_else(|| MuscleError::MissingArg("Missing required argument --destination".to_string()))?;
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--mode" => {
                ...
                else { return Err(MuscleError::MissingArg("Missing value for --mode".to_string())); }
            }
            other => {
                return Err(MuscleError::InvalidArg(format!("Unknown argument '{}'", other)));
            }
...
fn run_ftp_filter_helper(
    ...
    let output = cmd
        ...
        .arg(max_size_mb)
        .output()
        .map_err(|e| MuscleError::IoError(format!("Failed to start Python ftp_filter_helper: {}", e)))?;
    ...
    if output.status.success() {
        ...
        let stderr = String::from_utf8_lossy(&output.stderr);
        Err(MuscleError::IoError(format!("FTP/SFTP Filter execution failed: {}", stderr.trim())))
    }
}
...
pub fn handle_net_ftp_download_filtered(args: &[String]) -> Result<(), MuscleError> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--host" => { if i + 1 < args.len() { host = Some(&args[i+1]); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --host".to_string())); } }
            "--port" => { if i + 1 < args.len() { port = args[i+1].clone(); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --port".to_string())); } }
            "--user" => { if i + 1 < args.len() { user = Some(&args[i+1]); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --user".to_string())); } }
            "--password" => { if i + 1 < args.len() { password = Some(&args[i+1]); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --password".to_string())); } }
            "--remote-dir" | "--remote_dir" => { if i + 1 < args.len() { remote_dir = Some(&args[i+1]); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --remote-dir".to_string())); } }
            "--local-dir" | "--local_dir" => { if i + 1 < args.len() { local_dir = Some(&args[i+1]); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --local-dir".to_string())); } }
            "--max-age-hours" | "--max_age_hours" => { if i + 1 < args.len() { max_age_hours = args[i+1].clone(); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --max-age-hours".to_string())); } }
            "--min-age-hours" | "--min_age_hours" => { if i + 1 < args.len() { min_age_hours = args[i+1].clone(); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --min-age-hours".to_string())); } }
            "--min-size-mb" | "--min_size_mb" => { if i + 1 < args.len() { min_size_mb = args[i+1].clone(); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --min-size-mb".to_string())); } }
            "--max-size-mb" | "--max_size_mb" => { if i + 1 < args.len() { max_size_mb = args[i+1].clone(); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --max-size-mb".to_string())); } }
    ...
    let user = user.ok_or_else(|| MuscleError::MissingArg("Missing required argument --user".to_string()))?;
    let password = password.ok_or_else(|| MuscleError::MissingArg("Missing required argument --password".to_string()))?;
    let remote_dir = remote_dir.ok_or_else(|| MuscleError::MissingArg("Missing required argument --remote-dir".to_string()))?;
    let local_dir = local_dir.ok_or_else(|| MuscleError::MissingArg("Missing required argument --local-dir".to_string()))?;
...
pub fn handle_net_sftp_download_filtered(args: &[String]) -> Result<(), MuscleError> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--port" => { if i + 1 < args.len() { port = args[i+1].clone(); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --port".to_string())); } }
            "--user" => { if i + 1 < args.len() { user = Some(&args[i+1]); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --user".to_string())); } }
            "--password" => { if i + 1 < args.len() { password = args[i+1].clone(); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --password".to_string())); } }
            "--key-path" | "--key_path" => { if i + 1 < args.len() { key_path = args[i+1].clone(); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --key-path".to_string())); } }
            "--key-passphrase" | "--key_passphrase" => { if i + 1 < args.len() { key_passphrase = args[i+1].clone(); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --key-passphrase".to_string())); } }
            "--remote-dir" | "--remote_dir" => { if i + 1 < args.len() { remote_dir = Some(&args[i+1]); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --remote-dir".to_string())); } }
            "--local-dir" | "--local_dir" => { if i + 1 < args.len() { local_dir = Some(&args[i+1]); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --local-dir".to_string())); } }
            "--max-age-hours" | "--max_age_hours" => { if i + 1 < args.len() { max_age_hours = args[i+1].clone(); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --max-age-hours".to_string())); } }
            "--min-age-hours" | "--min_age_hours" => { if i + 1 < args.len() { min_age_hours = args[i+1].clone(); i += 2; } else { return Err(MuscleError::MissingArg("Missing value for --min-age-hours".to_string())); } }
    ...
    if secret.is_empty() {
        return Err(MuscleError::MissingArg("Either --password or --key-path must be specified".to_string()));
    }
